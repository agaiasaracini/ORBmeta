...
reORBgen_weight <- function(y, s, n1, n2, outcome, init_param, alpha, true.SE=NULL, LR.CI = FALSE, w) {
  ...
  if(outcome == "benefit"){
    ...
    #integral should be a function of y/logRR bc we integrate over y/logRR
    integrand <- function(y, mu, tau_squared, sigma_squared_imputed) {
      (1/sqrt(2*pi*(sigma_squared_imputed + tau_squared))) * exp(-0.5 * ((y - mu)^2 / (sigma_squared_imputed + tau_squared))) * (1/(1+exp(w*(y-z_alpha*sigma_squared_imputed))))
    }
    ...
    w_i_REML <- 1/(sigma_squared + tau.REML)
    mu.REML <- sum(w_i_REML*logRR)/sum(w_i_REML)
    ...
    w_i_REML_adj <- 1/(sigma_squared + tau.adj.REML)
    mu.adj.REML <- sum(w_i_REML_adj*logRR)/sum(w_i_REML_adj)
